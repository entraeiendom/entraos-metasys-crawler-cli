...
def test_login(logged_in_metasys_bearer, generate_token):
    assert logged_in_metasys_bearer.token == generate_token


def test_refresh(requests_mock, metasys_baseurl, logged_in_metasys_bearer, generate_token):
    ...
    logged_in_metasys_bearer.refresh()
    assert logged_in_metasys_bearer.token == new_token
    assert logged_in_metasys_bearer.expires == now_plus_one_hour


def test_validate(requests_mock, logged_in_metasys_bearer, generate_token):
    ...
    """
    logged_in_metasys_bearer.validate()
...
def test_validate_with_refresh(requests_mock, metasys_baseurl, logged_in_metasys_bearer, generate_token):
    """ We test validate on the bearer object. Since this is a brand new
    object nothing should happen. If this fails then something is wrong.
    """
    new_token = "snafu"
    # Make the token almost expire to trigger refresh:
    logged_in_metasys_bearer.expires = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(minutes=9)
...
def test_bearer_token_dunder_call(requests_mock, metasys_baseurl, generate_token, metasys_bearer):
    ...
    resp = requests.post(metasys_baseurl + '/make_unicorn', auth=metasys_bearer)
