...
def test_get_object(requests_mock, baseurl, logged_in_bearer):
    ...
    mockdb_session = AlchemyMagicMock()
...
def test_enrich_objects(requests_mock, baseurl, logged_in_bearer):
    ...
    mockdb_session = UnifiedAlchemyMagicMock()
    mockdb_session.add(return_obj)
    crawler.enrich_objects(mockdb_session, baseurl, logged_in_bearer, 0.0)
    # assert mockdb_session.
    assert mockdb_session.add.call_args[0][0].successes == 1
    assert json.loads(json_text) == json.loads(mockdb_session.add.call_args[0][0].response)
